...
extern UniValue DoZmyusSpend(const CAmount nAmount, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str);
...
extern UniValue setzmyusseed(const JSONRPCRequest& request);
extern UniValue getzmyusseed(const JSONRPCRequest& request);
extern UniValue generatemintlist(const JSONRPCRequest& request);
extern UniValue searchdzmyus(const JSONRPCRequest& request);
extern UniValue dzmyusstate(const JSONRPCRequest& request);
