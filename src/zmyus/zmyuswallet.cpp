...
CzMYUSWallet::CzMYUSWallet(CWallet* parent)
{
    this->wallet = parent;
    ...
    //Check for old db version of storing zMYUS seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZMYUSSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (wallet->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZMYUSSeed_deprecated()) {
                    LogPrintf("%s: Updated zMYUS seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zMYUS seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fRegtest && fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zMYUS wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzMYUSWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZMYUSCount(nCountLastUsed);
    else if (!walletdb.ReadZMYUSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzMYUSWallet::Lock()
{
    seedMaster.SetNull();
}

void CzMYUSWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzMYUSWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZMYUS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzMYUSWallet::LoadMintPoolFromDB()
{
...
void CzMYUSWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzMYUSWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzMYUSWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (wallet->zmyusTracker->HasPubcoinHash(pMint.first)) {
...
bool CzMYUSWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZMYUS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zMYUSTracker which also adds to database
    wallet->zmyusTracker->Add(dMint, true);
...
void CzMYUSWallet::SeedToZMYUS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
    libzerocoin::ZerocoinParams* params = Params().GetConsensus().Zerocoin_Params(false);
...
uint512 CzMYUSWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzMYUSWallet::GenerateDeterministicZMYUS(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzMYUSWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzMYUSWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzMYUSWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
