...
#include "zmyus/zmyuswallet.h"


CzMYUSTracker::CzMYUSTracker(CWallet* parent)
{
    this->wallet = parent;
...
CzMYUSTracker::~CzMYUSTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzMYUSTracker::Init()
{
...
bool CzMYUSTracker::Archive(CMintMeta& meta)
{
...
bool CzMYUSTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzMYUSTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzMYUSTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzMYUSTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzMYUSTracker::GetSerialHashes()
{
...
CAmount CzMYUSTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzMYUSTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzMYUSTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzMYUSTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzMYUSTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzMYUSTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzMYUSTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzMYUSTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzMYUSTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzMYUSTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(wallet->strWalletFile);
...
void CzMYUSTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzMYUSWallet* zMYUSWallet)
{
    bool iszMYUSWalletInitialized = (nullptr != zMYUSWallet);
    CMintMeta meta;
    ...
    if (!iszMYUSWalletInitialized)
        zMYUSWallet = new CzMYUSWallet(wallet);
    meta.isSeedCorrect = zMYUSWallet->CheckSeed(dMint);
    if (!iszMYUSWalletInitialized)
        delete zMYUSWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzMYUSTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMYUSTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzMYUSTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzMYUSTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzMYUSTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzMYUSTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (fUpdateStatus) {
        ...
        LogPrint(BCLog::LEGACYZC, "%s: added %d dzmyus from DB\n", __func__, listDeterministicDB.size());
    }
