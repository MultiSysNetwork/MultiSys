...
bool CLegacyZMyusStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZMYUSStake from non zc-spend", __func__);
    ...
    *this = CLegacyZMyusStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zMYUS stake origin", __func__);

    // All good
    return true;
}

CLegacyZMyusStake::CLegacyZMyusStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyZMyusStake::GetIndexFrom()
{
...
CAmount CLegacyZMyusStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZMyusStake::GetUniqueness() const
{
...
bool CLegacyZMyusStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zMYUS stake block: height %d outside range", __func__, nHeight);
