...
bool CMyusStake::InitFromTxIn(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend())
        return error("%s: unable to initialize CMYUSStake from zerocoin spend", __func__);
...
bool CMyusStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CMyusStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CMyusStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CMyusStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CMyusStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CMyusStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CMyusStake::GetUniqueness() const
{
    //The unique identifier for a MYUS stake is the outpoint
...
CBlockIndex* CMyusStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CMyusStake::ContextCheck(int nHeight, uint32_t nTime)
{
