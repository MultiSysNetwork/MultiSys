...
public:
    ...
    bool WriteZMYUSSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZMYUSSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZMYUSSeed_deprecated(uint256& seed);
    bool EraseZMYUSSeed();
    bool EraseZMYUSSeed_deprecated();

    bool WriteZMYUSCount(const uint32_t& nCount);
    bool ReadZMYUSCount(uint32_t& nCount);
