...
#include "zmyus/zmyusmodule.h"
#include "zmyus/zmyuswallet.h"
#include "zmyus/zmyustracker.h"
...
enum WalletFeature {
    ...
    FEATURE_PRE_MULTISYS = 61000, // inherited version..
...
// Possible states for zMYUS send
enum ZerocoinSpendStatus {
    ZMYUS_SPEND_OKAY = 0,                            // No error
    ZMYUS_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZMYUS_WALLET_LOCKED = 2,                         // Wallet was locked
    ZMYUS_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZMYUS_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZMYUS_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZMYUS_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZMYUS_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZMYUS_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZMYUS_TXMINT_GENERAL = 9,                        // General errors in MintsToInputVectorPublicSpend
    ZMYUS_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZMYUS_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZMYUS_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZMYUS_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZMYUS_SPENT_USED_ZMYUS = 14,                      // Coin has already been spend
    ZMYUS_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZMYUS_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void doZMyusRescan(const CBlockIndex* pindex, const CBlock& block, std::set<uint256>& setAddedToWallet, const Consensus::Params& consensus, bool fCheckZMYUS);
    ...
    bool CreateZMYUSOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    // zMYUS wallet
    std::unique_ptr<CzMYUSTracker> zmyusTracker{nullptr};
    void setZWallet(CzMYUSWallet* zwallet);
    CzMYUSWallet* getZWallet();
