...
#include "qt/multisys/forms/ui_topbar.h"
#include "qt/multisys/loadingdialog.h"
#include "qt/multisys/lockunlock.h"
#include "qt/multisys/qtutils.h"
#include "qt/multisys/receivedialog.h"
...
TopBar::TopBar(MULTISYSGUI* _mainWindow, QWidget* parent) : PWidget(_mainWindow, parent),
                                                        ui(new Ui::TopBar)
{
    ui->setupUi(this);
    ...
    setCssProperty({ui->labelPendingPiv, ui->labelImmaturePiv, ui->labelAvailablePiv,
                       ui->labelLockedPiv, ui->labelMasternodeCount, ui->labelCollateralMyus,
...
//    ui->pushButtonFAQ->setButtonText(tr("FAQ"));
    ...
    ui->pushButtonConf->setButtonText("multisys.conf");
...
void TopBar::onBtnConfClicked()
{
    ...
    if (!GUIUtil::openConfigfile())
        inform(tr("Unable to open multisys.conf with default application"));
}
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("MYUS")
    updateDisplayUnit();
...
void TopBar::refreshStatus()
{
    ...
    // Collateral
    ui->labelCollateralMyus->setText(GUIUtil::formatBalance(CMasternode::GetMasternodeNodeCollateral(chainActive.Tip()->nHeight), nDisplayUnit));
}
