...
#include "qt/multisys/forms/ui_snackbar.h"
#include "qt/multisys/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(MULTISYSGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &MULTISYSGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
