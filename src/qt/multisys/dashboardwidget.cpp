...
#include "qt/multisys/forms/ui_dashboardwidget.h"
#include "qt/multisys/sendconfirmdialog.h"
#include "qt/multisys/txrow.h"
...
DashboardWidget::DashboardWidget(MULTISYSGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of MYUS earned via Staking & Masternodes"));
	...
	ui->labelAmountMNRewards->setText("0 MYUS");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &MULTISYSGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
 ...
 //       connect(ui->pushImgEmpty, &QPushButton::clicked, window, &MULTISYSGUI::openFAQ);
...
void DashboardWidget::changeChartColors()
{
    ...
    if (isLightTheme()) {
        ...
        gridLineColorX = QColor(19,20,19);
        linePenColorY =  gridLineColorX;
        backgroundColor = linePenColorY;
    }
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair MYUS, zMYUS
