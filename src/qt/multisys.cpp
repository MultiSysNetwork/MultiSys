...
// Copyright (c) 2015-2020 The PIXV developers
// Copyright (c) 2021 The DECENOMY Core Developers
// Distributed under the MIT/X11 software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#if defined(HAVE_CONFIG_H)
#include "config/multisys-config.h"
#endif
...
#include "qt/multisys/splash.h"
#include "qt/multisys/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("multisys-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in multisys.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in multisys.qrc)
...
/** Class encapsulating MultiSys Core startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (MULTISYSGUI)
...
{
    window = new MULTISYSGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &MULTISYSGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &MULTISYSGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(MULTISYSGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(MULTISYSGUI::DEFAULT_WALLET);
        ...
        // MultiSys: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &MULTISYSGUI::handlePaymentRequest);
        connect(window, &MULTISYSGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. MultiSys can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(multisys_locale);
    Q_INIT_RESOURCE(multisys);
    ...
    /// 6. Determine availability of data directory and parse multisys.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("MultiSys"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("MultiSys"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // multisys: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("MultiSys didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
