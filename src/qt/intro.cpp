...
// Copyright (c) 2015-2019 The PIXV developers
...
static const uint64_t BLOCK_CHAIN_SIZE = 10LL * MB_BYTES;
                                ...
                                signalled(false)
{
    ...
    setCssProperty(ui->label_4, "text-intro-black");
    setCssProperty(ui->sizeWarningLabel, "text-intro-black");
    setCssProperty(ui->freeSpace, "text-intro-black");
    setCssProperty(ui->errorMessage, "text-intro-black");
...
bool Intro::pickDataDirectory()
{
    ...
    if (!fs::exists(GUIUtil::qstringToBoostPath(dataDir)) || GetBoolArg("-choosedatadir", DEFAULT_CHOOSE_DATADIR)) {
        ...
        while (true) {
            ...
            try {
                ...
                QMessageBox::critical(0, tr("MultiSys"),
     ...
     * override -datadir in the multisys.conf file in the default data directory
     * (to be consistent with multisysd behavior)
     */
...
void Intro::setStatus(int status, const QString& message, quint64 bytesAvailable)
{
    ...
    case FreespaceChecker::ST_ERROR:
        ...
        ui->errorMessage->setStyleSheet("QLabel { color: #26A911}");
        break;
    }
    ...
    if (status == FreespaceChecker::ST_ERROR) {
        ...
        QString freeString = tr("%1 GB of free space available").arg(bytesAvailable / MB_BYTES);
        if (bytesAvailable < BLOCK_CHAIN_SIZE) {
            freeString += " " + tr("(of %1 GB needed)").arg(BLOCK_CHAIN_SIZE / MB_BYTES);
