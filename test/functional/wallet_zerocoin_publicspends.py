...
class ZerocoinSpendTest(MultiSysTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zmyus_bal, myus_bal):
            zmyus_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zmyus_bal)
            myus_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], myus_bal)
            return zmyus_bal, myus_bal
        ...
        zmyus_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zmyus_balance, 6666)
        ...
        zmyus_balance, balance = check_balances(denom_2, zmyus_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zmyus_balance, balance = check_balances(denom_3, zmyus_balance, balance)
        ...
        _, _ = check_balances(denom_1, zmyus_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
