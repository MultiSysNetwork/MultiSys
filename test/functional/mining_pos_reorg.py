...
from test_framework.test_framework import MultiSysTestFramework
...
class ReorgStakeTest(MultiSysTestFramework):
    ...
    def check_money_supply(self, expected_myus, expected_zmyus):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected MYUS and zMYUS supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_myus))
            for denom in node['zMYUSsupply']:
                assert_equal(node['zMYUSsupply'][denom], DecimalAmt(expected_zmyus[denom]))
    ...
    def run_test(self):
        ...
        # Check MYUS and zMYUS supply at the beginning
        # ------------------------------------------
        # zMYUS supply: 2 coins for each denomination
        expected_zmyus_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # MYUS supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zmyus_supply)
        ...
        # Verify that MYUS and zMYUS supplies were properly updated after the spends and reorgs
        self.log.info("Check MYUS and zMYUS supply...")
        ...
        expected_zmyus_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zmyus_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zmyus_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zmyus_supply)
